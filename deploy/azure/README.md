...
- `provision-media-storage.sh` — optional, one-time: creates an Azure Blob Storage account + private `media` container for large media (video, and eventually any attachment past the 25MB Postgres-bytea ceiling in `server/routes/files.ts`), enables CORS so the browser can PUT/GET blobs directly instead of the bytes passing through the Node app, and wires `AZURE_STORAGE_CONNECTION_STRING` / `AZURE_STORAGE_CONTAINER` / `MEDIA_MAX_UPLOAD_MB` into the Web App's app settings. Reads `RG`/`LOCATION`/`APP_NAME` from `config.env` like every other script here — run `./recover-secrets.sh` first if you don't already have it. Safe to re-run (idempotent, same pattern as `provision.sh`); the generated storage account name is persisted to `.media-storage.env` so a second run reuses it instead of creating a new account. Provisioning the storage account alone doesn't make large uploads work — the app code that issues SAS URLs and streams to/from blob storage is separate follow-up work, not part of this script.
...
- `provision-playwright-mcp.sh` — optional, for the UI Validation Agent use case: deploys Microsoft's official Playwright MCP server (browser automation — navigate/click/screenshot) as an Azure Container Instance inside a private VNet subnet (no public IP possible in that mode — enforced by Azure, not just a flag), and VNet-integrates the App Service to reach it privately. It has no built-in auth, so it's deliberately never exposed to the public internet — run this only if you actually need real browser automation from an agent. Uses ACI rather than Container Apps specifically to avoid the AKS node-pool capacity dependency Container Apps has — the first version of this script hit a real `AKSCapacityHeavyUsage` error in centralus. Prints the container's internal IP to register via Astra's own "Add MCP Server" screen at the end; no code change to the app itself. That IP is stable across container restarts but changes if the container group is ever deleted and recreated (e.g. to change its image) — re-run the script's last step to get the new one.
