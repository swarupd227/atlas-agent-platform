...
- **Web Crawl Ingestion for Knowledge Bases**: When adding a web URL source to a KB, users can optionally enable crawling to follow same-domain links and ingest linked pages. Configurable crawl depth (1-3 levels) and max pages (up to 50). Crawled child sources are tagged with `metadata.crawledFrom` and shown with "via crawl" badges. Parent source shows crawl progress (pending/crawling/complete with page counts). Backend performs BFS traversal with deduplication and same-domain filtering.
