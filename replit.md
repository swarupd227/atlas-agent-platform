...
- **Eval-Driven Operational Intelligence Fixes**: Two gaps closed in the eval→operations feedback loop: (1) **Server-Side Eval Gate on Deployment Promotion** — `POST /api/deployments/:id/promote` now hard-blocks production promotions when any eval suite pass rate < 80% (returns 400 with `evalGateBlocked`, `threshold`, `failingSuites`). Pilot promotions warn at < 60%. `bypassEvalGate: true` option allows override with audit trail logging. Readiness endpoint (`GET /api/deployments/:id/readiness`) now uses minimum pass rate across ALL suites (not best suite), aligning with promote gate semantics, and marks eval check as `enforced: true`. Frontend PromoteDialog shows enforced badges, eval gate error panels, and bypass workflow that still respects non-eval failing checks. (2) **Direct Eval Failure → KB Gap Analysis** — `GET /api/agents/:agentId/eval-kb-gaps` analyzes failed eval test cases across recent runs, extracts input/failure terms, matches against per-KB chunk content corpora (not combined), and returns per-gap KB attribution (`linkedKbIds` per gap). KB detail page has "Eval KB Gaps" tab showing gaps relevant to that specific KB. Agent detail KB section shows "Eval-Driven Gaps" badge with gap count.
