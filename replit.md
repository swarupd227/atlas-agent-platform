...
- **Root Cause Classification Engine**: Classifies agent drift into structured categories by analyzing eval history, KB staleness, and other factors, with results stored in `diagnosisDetails`.
- **Shadow Replay Validation Gate**: Creates shadow replay jobs linked to healing pipelines to validate remediation, blocking progression if replay fails.
...
- **Healing Intelligence Summary**: Provides a top-level summary of root cause classification, shadow replay, and context adjustment statuses for healing pipelines.
