...
- **Full-Screen Export Page**: Agent code export moved from a Dialog in agent-detail.tsx to a dedicated full-screen page at `/agents/:id/export` (`client/src/pages/agent-export.tsx`). Three-step flow (Configure → Preview → Deliver) with top nav bar showing agent name breadcrumb, clickable step progress, and action buttons. Configure step uses 4-column form layout with 2-column content (Tool Adapters + Dependencies/Env Vars). Preview step has collapsible file tree sidebar, full-height Monaco editor with file path bar and metadata badges. Deliver step shows 3-column delivery method cards (ZIP, Git, Replit). All existing functionality preserved: AI generation, file editing, ZIP download, Git push, approval requests, compile/eval gates.

- **Blueprint Library Enrichment**: Blueprints now support `patternType` (RAG Pipeline, Orchestrator, Fan-out, Linear Chain, Human-in-Loop, Custom), `tags` (text array), `isShared` (org sharing), and `forkedFromId` (provenance tracking). Agents have a `blueprintId` foreign key for usage tracking. The blueprints list page uses a rich card grid with stats bar, pattern/status/visibility filters, node flow chain visualization, validation health badges, and inline clone. Blueprint detail page includes Share toggle, Forked badge, and a settings panel for pattern type and tags when no node is selected.
- **Blueprint-First Agent Creation**: Blueprints are integrated into all 5 agent creation paths. Agent Wizard has a new "Choose Blueprint" step (step 2) with compact card grid grouped by pattern type. Templates support a `defaultBlueprintId` field that pre-selects a blueprint when creating agents from templates. The propose-agents AI pipeline post-processes proposals with `suggestedPatternType` and `suggestedBlueprintId` based on role/keyword matching. Agent proposal cards display pattern type (violet badge) and blueprint name (blue badge) chips. POST /api/agents and bulk-create routes accept optional `blueprintId` and auto-resolve blueprint JSON. Template detail page has a "Default Blueprint" section with dropdown selector.
