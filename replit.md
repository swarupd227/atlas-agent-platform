...
- **Code Generation / Export**: Dual-mode agent deployment — config-driven runtime execution (existing) + code generation/export for standalone deployment using Ralph Loop pattern. Three entry points: (1) Blueprint → Export as Code wizard with format (TypeScript/Python), LLM provider (OpenAI/Anthropic Claude), Ralph Loop config (max iterations, completion promise), preview panel, and download; (2) Tool Catalog → Generate Adapter for standalone tool adapter code stubs; (3) Create Release wizard → Deploy as Source Package toggle for CI/CD deployment. Backend: POST /api/agents/:id/export-code, POST /api/tool-connectors/:id/generate-adapter.
