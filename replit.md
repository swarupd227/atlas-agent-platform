...
- **Agent Lifecycle Management**: Comprehensive tools for creation, deployment, monitoring, and governance.
- **Real Agent Runtime**: Deployed agents execute blueprints as background workers, resolving API calls through registered integrations with compliance checks.
- **Validation and Deployment Safety**: Features like Shadow Replay Studio for zero-risk validation and Canary Deployment Console for graduated rollouts.
...
- **Optimization & Healing**: Includes autonomous optimization, self-healing mechanisms, and AI-proposed changes.
- **Knowledge Management**: Supports vector-embedded document collections for RAG grounding, with web crawl ingestion and structured data import.
...
- **Deployment Activation Flow**: Pending/inactive deployments can be activated from the release detail page.
...
- **Full-Screen Export Page**: Dedicated full-screen page for agent code export with configure, preview, and deliver steps. Includes file search palette, per-file regeneration, diff view, dynamic deployment checklist, and export presets.
- **Table-Aware Document Chunking**: Knowledge base ingestion preserves tabular data structure from Word documents and HTML.
...
- **Unified Workflow State (UWS)**: Typed, persistent, reducer-based workflow state across pipeline stages, utilizing `workflow_state_schemas` and `workflow_state_checkpoints` tables.
