...
- **AI Enhance for Test Case Drafts**: `POST /api/ai/enhance-test-case-draft` accepts `{name, inputScenario, industry, useCase}` and uses OpenAI to generate enhanced inputScenario, expectedBehavior, difficultyTier, scenarioCategory, evaluationCriteria, rubricScoring, and tags. The "Add Test Case" dialog in Golden Dataset Detail has an "AI Enhance" button (enabled when Name + Input Scenario are filled) that auto-populates all remaining fields with AI-generated content.
