...
- **Code Generation / Export**: Dual-mode agent deployment — config-driven runtime execution (existing) + code generation/export for standalone deployment using Ralph Loop pattern. Export Wizard is an 11-step dialog: Step 1 (Export Type — "Managed Runtime Artifact" default vs "Source Export" for CI/CD and portability), Step 2 (Configure — Template & Framework selector with 7 options: Generic Tool-Calling Runtime (recommended), LangGraph Graph App, CrewAI Project Scaffold, Microsoft Foundry Agent, AWS Bedrock Agent, N8N Workflow, GCP Vertex AI Agent; language TypeScript/Python, LLM provider OpenAI/Anthropic Claude, Ralph Loop config with max iterations + completion promise; dynamic source file list per framework), Step 3 (Tool Adapter Resolution — shows every blueprint tool reference with status: "Built-in adapter included" / "Customer adapter required" / "Stub will be generated"; actions: "Generate Stub" creates placeholder code, "Attach Existing Adapter" pulls from ALMP tool registry, "Use Stub Instead" switches a built-in to stub; summary badges show counts per status; stub info notice warns about placeholder implementations), Step 4 (Dependencies & Reproducibility — dependency file preview (requirements.txt or package.json) with exact pinned versions by default, "pin versions" toggle to switch between exact pins and semver ranges, "minimize dependencies" warnings and count for optimization guidance, reproducible build notice when pinned), Step 5 (Environment Variables & Secrets Map — required env vars computed from LLM provider, framework, tool adapters, agent config, and OTEL settings; grouped by category: LLM Provider, Tool Credentials, Infrastructure, Agent Config; .env.example preview with empty values only, copy button; Replit secrets guidance for encrypted secrets; security notice about never committing real values), Step 6 (Observability & Trace Settings — OpenTelemetry-compatible traces on/off toggle, span granularity selector: per-node or per-tool-call, span mapping preview showing root/node/tool/llm spans, OTEL endpoint notice; conditionally adds OTEL_EXPORTER_OTLP_ENDPOINT and OTEL_SERVICE_NAME to env vars), Step 7 (Build & Test Gate — "Run static compile" and "Run eval suite locally" actions with status indicators idle/running/passed/failed, result output panels; calls POST /api/agents/:id/export-validate), Step 8 (Preview — file tabs with generated source code, download), Step 9 (Approval Preview — auto-generated approval requirement panel showing risk tier computed from export config, change summary, tool permissions with new stub warnings, validation gate results; "Request Export Approval" creates approval with objectType="export_package" and full evidence bundle including file tree, file contents, tool adapters, gate results; links to approval detail page), Step 10 (Delivery Target — three options: Download ZIP (always available), Push to Git Repo (repo URL input for CI/CD), Publish to Replit via GitHub import (step-by-step guidance); auto-downloads ZIP when selected), Step 11 (Deployment Guide — target-specific instructions: Replit hosting shows recommended deployment types (Autoscale/Reserved VM/Scheduled), publish path, secrets configuration via Replit Secrets tool, monitoring guidance; Git shows CI/CD setup steps and env config; ZIP shows getting started steps; includes "Export Complete" confirmation notice). Three entry points: (1) Blueprint → Export as Code button; (2) Implementation Graph → Generate Export Package button; (3) Summary tab → Export / Generate Code button. Implementation Graph on Blueprint tab shows declarative vs code-required components before export. Backend: POST /api/agents/:id/export-code (accepts framework param, toolAdapters record, pinVersions boolean, otelEnabled boolean, spanGranularity string), POST /api/agents/:id/export-validate (accepts type compile/eval, format, framework, llmProvider), POST /api/tool-connectors/:id/generate-adapter.
