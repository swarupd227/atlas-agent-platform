...
function SpanTreeNode({
  ...
  return (
    <div>
      <div
        className={`flex items-center gap-1 px-2 py-1.5 rounded-md cursor-pointer select-none group transition-colors text-xs ${
        ...
        <div className="px-3 pb-3 border-t bg-muted/20 pt-2">
          {judgeModel && (
            <div className="flex items-center gap-1 mb-1.5 text-muted-foreground">
              ...
              <span className="text-[10px]">{judgeModel}</span>
            </div>
          )}
          {reason && (
            <div className="bg-muted/50 rounded-md p-2 text-[11px] leading-relaxed whitespace-pre-wrap text-foreground/80 max-h-40 overflow-y-auto">
              {reason}
            </div>
          )}
          {!reason && !judgeModel && (
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Select a completed run for the same agent to compare spans and scores.
          </DialogDescription>
        </DialogHeader>
        <div className="flex flex-col gap-1 max-h-64 overflow-y-auto py-1">
          {candidates.length === 0 ? (
            ...
            candidates.map((r) => (
              ...
              >
                <div className="font-medium">{r.id.slice(0, 12)}…</div>
...
function SpanDiffRow({
  ...
  return (
    <tr className={changed ? "bg-amber-500/5" : ""}>
      <td className="px-3 py-1.5 text-[10px] font-medium text-muted-foreground w-24 align-top">{label}</td>
      <td className={`px-3 py-1.5 text-[11px] font-mono max-w-[200px] align-top ${changed ? "text-amber-700 dark:text-amber-400" : ""}`}>
        {leftVal || "—"}
      </td>
      <td className={`px-3 py-1.5 text-[11px] font-mono max-w-[200px] align-top ${changed ? "text-amber-700 dark:text-amber-400" : ""}`}>
        {rightVal || "—"}
...
export default function EvalTraceInspector() {
  ...
  const pinMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!res.ok) throw new Error("Failed to pin trace");
      return res.json();
    },
    onSuccess: () => {
  ...
  const selectedSpan = useMemo(() => {
    ...
    const spanId = selectedSpanId ?? trace.rootSpanId ?? trace.spans?.[0]?.id ?? null;
    if (!spanId) return null;
    return trace.spans?.find((s) => s.id === spanId) ?? null;
  ...
  // ── Span diff comparison data ────────────────────────────────────────────

  const compareSpanMap = useMemo(() => {
    if (!compareTrace?.spans) return new Map<string, EvalSpan>();
    return new Map(compareTrace.spans.map((s) => [s.name, s]));
  }, [compareTrace]);
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden">
      {/* ── Top bar ──────────────────────────────────────────────────────────── */}
      <div className="shrink-0 border-b bg-background px-4 py-3 flex items-center gap-3 flex-wrap">
        {/* Breadcrumb */}
        ...
        <div className="flex items-center gap-1.5 text-xs">
          ...
          <span className="font-medium text-sm">{agent?.name ?? `Agent ${(run?.agentId ?? "").slice(0, 8)}`}</span>
          {run?.agentVersion && (
        ...
        {golden?.input && (
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground max-w-[280px]">
            ...
            <span className="truncate italic">&ldquo;{golden.input.slice(0, 80)}{golden.input.length > 80 ? "…" : ""}&rdquo;</span>
          </div>
        )}
        ...
        <div className="flex items-center gap-3 text-xs text-muted-foreground">
          ...
          {trace.createdAt && (
            <span className="hidden lg:block">{formatDate(trace.createdAt as string)}</span>
          )}
        </div>
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        ...
        {/* ── Center: Span detail ───────────────────────────────────────────── */}
        <div className="flex-1 overflow-y-auto p-4 min-w-0" data-testid="span-detail">
          {!selectedSpan ? (
            ...
            <div className="max-w-2xl">
              ...
              {compareTrace && (
                <div className="mt-6 border rounded-lg overflow-hidden">
                  <div className="flex items-center gap-2 bg-amber-500/10 border-b border-amber-500/20 px-3 py-2">
                    <GitCompare className="w-4 h-4 text-amber-600" />
                    <span className="text-xs font-medium text-amber-700 dark:text-amber-400">
                      Comparison — {compareTrace.runId.slice(0, 12)}
                    </span>
                    <button
                      onClick={() => setCompareTraceId(null)}
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs">
                      ...
                      <tbody className="divide-y">
                        <SpanDiffRow
                          label="Pass/Fail"
                          leftVal={trace.passFail == null ? "—" : trace.passFail ? "Pass" : "Fail"}
                          rightVal={compareTrace.passFail == null ? "—" : compareTrace.passFail ? "Pass" : "Fail"}
                          changed={trace.passFail !== compareTrace.passFail}
                        />
                        <SpanDiffRow
                          label="Latency"
                          leftVal={trace.latencyMs != null ? `${trace.latencyMs}ms` : "—"}
                          rightVal={compareTrace.latencyMs != null ? `${compareTrace.latencyMs}ms` : "—"}
                          changed={trace.latencyMs !== compareTrace.latencyMs}
                        />
                        <SpanDiffRow
                          label="Tokens"
                          leftVal={trace.totalTokens != null ? String(trace.totalTokens) : "—"}
                          rightVal={compareTrace.totalTokens != null ? String(compareTrace.totalTokens) : "—"}
                          changed={trace.totalTokens !== compareTrace.totalTokens}
                        />
                        {/* Per-metric score deltas */}
                        {Object.entries(parseScores(trace.scores)).map(([metric, entry]) => {
                          const baselineEntry = (parseScores(compareTrace.scores) as Record<string, ScoreEntry>)[metric];
                          const cur = scoreValue(entry);
                          const bas = baselineEntry ? scoreValue(baselineEntry) : null;
                          const changed = cur !== bas;
                          const delta = cur != null && bas != null ? cur - bas : null;
                          const curStr = cur != null ? `${(cur * 100).toFixed(0)}%${delta != null ? ` (${delta > 0 ? "+" : ""}${(delta * 100).toFixed(1)}%)` : ""}` : "—";
                          const basStr = bas != null ? `${(bas * 100).toFixed(0)}%` : "—";
                          return (
                            <SpanDiffRow
                              key={metric}
                              label={metric}
                        ...
                        {/* Spans present in baseline but not current */}
                        {(compareTrace.spans ?? [])
                          .filter((s) => !(trace.spans ?? []).find((cs) => cs.name === s.name))
                          .map((s) => (
                            <tr key={s.id} className="bg-amber-500/5">
                              <td className="px-3 py-1.5 text-[10px] font-medium text-muted-foreground">Span</td>
                              <td className="px-3 py-1.5 text-[10px] text-muted-foreground italic">not present</td>
                              <td className="px-3 py-1.5 text-[10px] text-amber-600">{s.name}</td>
                            </tr>
                          ))}
                        {/* Spans in current but not baseline */}
                        {(trace.spans ?? [])
                          .filter((s) => !compareSpanMap.has(s.name))
                          .map((s) => (
                            <tr key={s.id} className="bg-amber-500/5">
                              <td className="px-3 py-1.5 text-[10px] font-medium text-muted-foreground">Span</td>
                              <td className="px-3 py-1.5 text-[10px] text-amber-600">{s.name} (new)</td>
                              <td className="px-3 py-1.5 text-[10px] text-muted-foreground italic">not present</td>
                            </tr>
                          ))}
                      </tbody>
        ...
        {/* ── Right: Metric panel ──────────────────────────────────────────── */}
        <div className="w-72 shrink-0 border-l flex flex-col overflow-hidden" data-testid="metric-panel">
              ...
              {scoreEntries.length === 0 ? (
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mb-3 p-2 bg-muted/30 rounded-md">
                    ...
                    <span className="ml-auto text-[10px] text-muted-foreground">{scoreEntries.length} metric{scoreEntries.length !== 1 ? "s" : ""}</span>
                  </div>
                    ...
                    .map(([name, entry]) => {
                      ...
                      return (
                        <div key={name} data-testid={`reasoning-${name}`}>
                          ...
                          <div className="bg-muted/40 rounded-md p-2.5 text-[11px] leading-relaxed whitespace-pre-wrap text-foreground/80">
                            {scoreReason(entry)}
                          </div>
                        </div>
                      );
