...
export default function BlackRock2Demo() {
  ...
  const startLiveRun = useCallback((scenarioId: string) => {
    ...
    es.addEventListener("run_start", (e) => {
      ...
      addEvent("run_start", "Astra Agents Runtime", `Live run started — scenario: ${d.scenarioId}`);
    });
    ...
    es.addEventListener("run_complete", (e) => {
      const d = JSON.parse(e.data);
      addEvent("run_complete", "Astra Agents Runtime", `All 6 agents completed — ${d.caseId} — traces available in Runs & Traces`);
  ...
  const modalHasExc       = (modalSnap?.summaryStats?.openExceptions ?? 0) > 0;
        ...
        <div className="flex items-center justify-between gap-3 px-5 py-3 border-b">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted-foreground leading-tight">BlackRock Investment Management · Astra Agents Agent Orchestration</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <AgentTeamCard />
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex items-center justify-between gap-3 px-5 py-3 border-b bg-background shrink-0">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium">Live Astra Agents Runtime</span>
              {running && (
                <div className="flex items-center gap-1.5">
          ...
          <div className="flex flex-col text-sm">
            ...
            <div className="bg-[#1a1a2e] text-white px-6 py-4 rounded-t-lg">
              ...
              <div className="space-y-1.5 text-[12px] text-slate-300">
                <div className="flex gap-2"><span className="text-slate-500 w-14 shrink-0">From</span><span>{modalSnap.fromAddress ?? "ASTRA AGENTS AIM <aim-noreply@blackrock.com>"}</span></div>
            ...
            <div className="bg-background px-6 py-5 space-y-5 rounded-b-lg">
              ...
              <div className="border-t pt-4 text-[10px] text-muted-foreground space-y-1">
                <p>Generated by BlackRock ASTRA AGENTS Agent · AIM Identity &amp; Access Management</p>
                <p className="font-mono text-muted-foreground/50">Ref: {modalSnap.messageId ?? "—"}</p>
              </div>
            </div>
          </div>
        )}
