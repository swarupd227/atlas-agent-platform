...
export default function AdvSupportS2Resolution({ state }: Props) {
  ...
  const confidencePct = isDone ? Math.round(state.metrics.kb_confidence * 100) : 0;
  const gateResult    = isDone ? (state.metrics.kb_confidence >= 0.65 ? "passed" : "failed") : "pending";
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {/* Running placeholder */}
      {isRunning && (
        <div className="flex flex-col items-center justify-center gap-3 py-12 text-center" data-testid="kb-running-placeholder">
          <Cpu className="w-8 h-8 animate-pulse" style={{ color: `${ACCENT}80` }} />
          <div className="text-sm text-muted-foreground/60 font-medium">SUP-002 searching KB…</div>
          <div className="text-xs text-muted-foreground/30">KB results will appear once the search completes</div>
        </div>
      )}

      {/* All KB intelligence — only shown after SUP-002 completes */}
      {isDone && (
        <>
          {/* T1 Autonomous Resolution success — Scenario B only */}
          {sc === "B" && gateResult === "passed" && (
            <div
                  ...
                  <div key={r.id} className="flex items-start gap-2 py-1.5 border-b border-border/20 last:border-0" data-testid={`kb-result-${r.id}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-1">
                        ...
                        <span className="text-[10px] font-mono shrink-0" style={{ color: ACCENT }}>{r.relevance.toFixed(2)}</span>
                      </div>
                  ...
                  <div key={r.id} className="flex flex-col gap-1 py-2 border-b border-border/20 last:border-0" data-testid={`hist-result-${r.id}`}>
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[10px] font-mono" style={{ color: ACCENT }}>{r.similarity.toFixed(2)}</span>
                      </div>
                    </div>
          ...
          <div className="rounded-lg border border-border/40 bg-card/50 p-4" data-testid="confidence-gate">
            ...
            <div className="flex items-center gap-4 mb-3">
              ...
              <span className="text-lg font-bold font-mono" style={{ color: confidencePct >= 65 ? "#22c55e" : "#ef4444" }}>
                {confidencePct}%
              </span>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 text-center">
              {[
                ...
                { label: "<65%",  desc: "→ Diagnostic",       active: confidencePct < 65,                              color: "text-rose-400"   },
              ].map(t => (
                <div
                  key={t.label}
