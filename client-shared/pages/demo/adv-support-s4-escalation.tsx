import { Package, Briefcase, ArrowRightCircle, Bell, ClipboardList, CheckCircle2, ExternalLink, ShieldAlert } from "lucide-react";
...
export default function AdvSupportS4Escalation({ state }: Props) {
  ...
  const caseNumber  = isC ? "00078034"                        : "00074821";
  const caseId      = isC ? "SF-CASE-2026-078034"             : "SF-CASE-2026-074821";
  const caseUrl     = isC
    ? "advantive.lightning.force.com/r/Case/00078034"
    : "advantive.lightning.force.com/r/Case/00074821";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="rounded-lg border border-border/40 bg-card/50 p-4" data-testid="salesforce-case">
          <div className="flex items-center gap-2 mb-3">
            ...
            {sfCreated && <Badge variant="outline" className={`ml-auto text-[9px] ${isC ? "border-rose-500/40 text-rose-300" : "border-emerald-500/40 text-emerald-400"}`}>{caseId}</Badge>}
          </div>
          {isDone ? (
            ...
            {isDone ? (
              <div className="flex flex-col gap-2">
                ...
                <div className="text-[10px] text-muted-foreground mt-1">
                  {isC
                    ? "P0 paged — Sofia Rodriguez is the primary ParityFactory FDA compliance specialist."
            ...
            {isDone ? (
              <div className="flex flex-col gap-1 text-xs">
                <div className="flex justify-between">
                  ...
                  <span>{isC ? "Tyler Brooks" : "James Whitfield"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Channel</span>
                  <span>{isC ? "Email + Slack + Phone" : "Email + Slack"}</span>
                </div>
                {isC && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Legal CC</span>
                    <span className="text-rose-300 font-mono text-[10px]">compliance@advantive.com</span>
                  </div>
                )}
                <div className="flex justify-between">
