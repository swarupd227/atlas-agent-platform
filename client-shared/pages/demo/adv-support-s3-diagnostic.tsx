...
export default function AdvSupportS3Diagnostic({ state }: Props) {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {/* Running placeholder */}
      {isRunning && (
        <div className="flex flex-col items-center justify-center gap-3 py-12 text-center" data-testid="diagnostic-running-placeholder">
          <Cpu className="w-8 h-8 animate-pulse" style={{ color: `${ACCENT}80` }} />
          <div className="text-sm text-muted-foreground/60 font-medium">SUP-003 analysing logs…</div>
          <div className="text-xs text-muted-foreground/30">Diagnostic intelligence will appear once log analysis completes</div>
        </div>
      )}

      {/* All diagnostic intelligence — only after SUP-003 completes */}
      {isDone && (
        <>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                      ...
                      <div key={e.code} className="flex items-center gap-3 py-1.5 border-b border-border/20 last:border-0" data-testid={`error-${e.code}`}>
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-3 mt-0.5">
                            <span className="text-[10px] text-muted-foreground">Count: <span className="font-mono text-foreground">{e.count}</span></span>
                            <span className="text-[10px] text-muted-foreground">Rate: <span className="font-mono text-foreground">{e.rate}</span></span>
                          </div>
                        </div>
                      </div>
                    ))}
            ...
            <div className="rounded-lg border border-border/40 bg-card/50 p-4" data-testid="error-pattern">
              <div className="flex items-center gap-2 mb-3">
                ...
                <Badge variant="outline" className="ml-auto text-[9px]" style={{ borderColor: `${ACCENT}50`, color: ACCENT }}>{isC ? "0.94" : "0.97"} confidence</Badge>
              </div>
              <div className="flex flex-col gap-2">
                {isC ? (
                  <>
