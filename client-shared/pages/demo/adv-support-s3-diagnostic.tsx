import { Microscope, FileSearch, Database, CheckCircle2, AlertTriangle, ChevronRight, ShieldAlert } from "lucide-react";
                ...
                {isDone && (
                  <div className="mt-2 rounded border border-amber-500/20 bg-amber-950/20 px-3 py-2">
                    ...
                    <div className="text-[10px] text-muted-foreground">
                      ParityFactory sync daemon exited silently (code 0) after DB maintenance connection pool recycle.
                      Batch records <span className="font-mono text-amber-300">LOT089-094</span> were in the sync buffer but not committed to the FDA-validated repository.
                      Root cause: <span className="text-rose-400 font-medium">DaemonAutoRestart=false</span> (PF-BUG-821-0033).
                    </div>
                  </div>
                )}
              </>
            ...
            <div className="flex flex-col gap-2">
              {isC ? (
                <>
                  ...
                  <div className="flex items-center justify-between"><span className="text-xs text-muted-foreground">Remote Access</span><span className="text-[11px] text-muted-foreground">Not required</span></div>
