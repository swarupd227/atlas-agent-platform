#!/usr/bin/env bash
# =============================================================================
# ATLAS — HNP Government Beat Intelligence (HNP-GOVT) — Dev Provisioning
...
#   bash provision_hnp_govt_dev.sh
#
# Creates via Platform APIs only (no direct DB writes):
#   • 4 Knowledge Bases
#   • 4 MCP Servers + 17 Tools
#   • 6 Skills
#   • 1 Outcome Contract
#   • 4 Governance Policies
#   • 6 Ontology Concepts (Texas govt entities)
#   • 4 Agents (with system prompts, KBs, MCP servers, skills, policies, blueprint)
#   • 1 Eval Suite
#   • 1 Blueprint
#   • 4 Deployments (so live-run runtime can attach immediately)
#
...
# ─── STEP 1: Create 4 Knowledge Bases ────────────────────────────────────────
...
# ─── STEP 2: Create 4 MCP Servers + tools ────────────────────────────────────
echo "STEP 2: Creating 4 MCP Servers..." >&2

MCP_ASSEMBLY=$(post_inline "MCP: HNP Assembly" "/api/mcp-servers" "$(jq -n --arg url "${BASE_URL}/api/mock/hnp-assembly" '{
...
MCP_KB=$(post_inline "MCP: HNP Knowledge Base" "/api/mcp-servers" "$(jq -n --arg url "${BASE_URL}/api/mock/hnp-knowledge-base" '{
...
MCP_PR=$(post_inline "MCP: HNP Public Records" "/api/mcp-servers" "$(jq -n --arg url "${BASE_URL}/api/mock/hnp-public-records" '{
...
MCP_CMS=$(post_inline "MCP: HNP CMS" "/api/mcp-servers" "$(jq -n --arg url "${BASE_URL}/api/mock/hnp-cms" '{
  name: "HNP CMS MCP",
...
# ─── STEP 3: Register tools on each MCP server ───────────────────────────────
echo "STEP 3: Registering tools on each MCP server..." >&2

post_tool() {
  local server_id="$1" name="$2" desc="$3" risk="$4" schema="$5"
  # Auto-derive HTTP endpoint + method from tool name so the agent runtime
  # can call the mock MCP server's hyphenated route. Without these annotations,
  # callMcpTool() falls back to the bare baseUrl which returns Vite HTML.
...
# ─── STEP 7: Create Ontology Concepts (Texas govt) ───────────────────────────
echo "STEP 7: Creating 6 Ontology Concepts (Texas government)..." >&2
...
ONT_COUNT=$(echo "$ONT_RESP" | jq -r '(.concepts // []) | length')
...
# System prompts (kept compact — full ones come from shared-defs file at runtime
# via runtimeConfig.prompt; the agent.systemPrompt below is the durable role
# definition stored on the agent record).
...
# ─── STEP 9: Wire each agent (PATCH skills, policies, KB-RAG, runtime, blueprint) ──
...
# Compact blueprint (same DAG for all 4 — they share the pipeline)
...
# Full prompts (the runtime uses these — they include scenario context)
...
link_kb() {
  ...
  if [ -z "$link_id" ]; then
    echo "    ⚠ KB link skipped (may already exist): $agent_id ↔ $kb_id :: $response" >&2
    return 0
  fi
...
# ─── STEP 11: Link MCP servers to agents ─────────────────────────────────────
...
link_mcp() {
  ...
  link_id=$(echo "$response" | jq -r '.id // empty')
  if [ -z "$link_id" ]; then
    echo "    ⚠ MCP link skipped (may already exist): $agent_id ↔ $server_id :: $response" >&2
    return 0
  fi
...
# Wire eval suite to all 4 agents
for AID in "$A1" "$A2" "$A3" "$A4"; do
  curl -s -X PATCH "${BASE_URL}/api/agents/${AID}" \
    -H "Content-Type: application/json" \
    -d "{\"evalBindings\":{\"suites\":[{\"suiteId\":\"${EVAL_SUITE}\",\"schedule\":\"on_deploy\",\"environment\":\"staging\"}]}}" > /dev/null
done
echo "  ✓ Eval suite wired to all 4 agents" >&2
echo "" >&2

# ─── STEP 13: Create deployments (so live-run runtime can attach immediately) ──
echo "STEP 13: Creating deployments for each agent..." >&2
...
create_deploy() {
  ...
  if [ -z "$did" ]; then
    echo "    ⚠ deployment create returned no id (may already exist): $response" >&2
    return 0
  fi
...
# ─── SUMMARY ─────────────────────────────────────────────────────────────────
echo "==================================================" >&2
echo " HNP-GOVT DEV PROVISIONING COMPLETE" >&2
echo "==================================================" >&2
echo "" >&2
echo "Knowledge Bases:" >&2
echo "  Assembly Corpus Index:           $KB1" >&2
echo "  Investigative Journalism Stds:   $KB2" >&2
echo "  Texas Government Entity Ont:     $KB3" >&2
echo "  Elected Official Profile Index:  $KB4" >&2
echo "" >&2
echo "MCP Servers:" >&2
echo "  Assembly:        $MCP_ASSEMBLY" >&2
echo "  Knowledge Base:  $MCP_KB" >&2
echo "  Public Records:  $MCP_PR" >&2
echo "  CMS:             $MCP_CMS" >&2
echo "" >&2
...
echo "" >&2
echo "Live demo endpoint:" >&2
echo "  curl -N \"$BASE_URL/demo-api/hnp-govt/live-run?scenario=happy\"" >&2
echo "  curl -N \"$BASE_URL/demo-api/hnp-govt/live-run?scenario=attribution-block\"" >&2
echo "  curl -N \"$BASE_URL/demo-api/hnp-govt/live-run?scenario=foia-routing-fail\"" >&2
echo "==================================================" >&2

# Export IDs (machine-readable on stdout for migration script consumption)
echo "HNP_GOVT_KB1=$KB1"
echo "HNP_GOVT_KB2=$KB2"
echo "HNP_GOVT_KB3=$KB3"
echo "HNP_GOVT_KB4=$KB4"
echo "HNP_GOVT_MCP_ASSEMBLY=$MCP_ASSEMBLY"
echo "HNP_GOVT_MCP_KB=$MCP_KB"
echo "HNP_GOVT_MCP_PR=$MCP_PR"
echo "HNP_GOVT_MCP_CMS=$MCP_CMS"
echo "HNP_GOVT_S1=$S1"
echo "HNP_GOVT_S2=$S2"
echo "HNP_GOVT_S3=$S3"
echo "HNP_GOVT_S4=$S4"
...
echo "HNP_GOVT_P3=$P3"
echo "HNP_GOVT_P4=$P4"
echo "HNP_GOVT_OUTCOME=$OUTCOME"
echo "HNP_GOVT_A1=$A1"
echo "HNP_GOVT_A2=$A2"
echo "HNP_GOVT_A3=$A3"
echo "HNP_GOVT_A4=$A4"
echo "HNP_GOVT_EVAL_SUITE=$EVAL_SUITE"
