...
async function migrateAgent(agent: typeof AGENTS[0]): Promise<Record<string, string>> {
  ...
  for (const policy of agent.policies) {
    const r = await prodPost("/api/governance/policies", {
      name: policy.name,
  ...
  // 2. Knowledge Base
  console.log("\n  2. Knowledge Base");
  const kb = await prodPost("/api/knowledge-bases", {
    name: agent.kbName,
  ...
  console.log(`    ✓ ${agent.kbName} → ${kb.id}`);
  ids.kbId = kb.id;
  ...
  for (const skill of agent.skills) {
    const r = await prodPost("/api/skills", {
      name: skill.name,
  ...
  // 4. Blueprint
  console.log("\n  4. Blueprint");
  const bp = await prodPost("/api/blueprints", {
    name: `${agent.code} — ${agent.stage} Blueprint`,
  ...
  ids.blueprintId = bp.id;

  // 5. MCP Server + Tools
  console.log("\n  5. MCP Server");
  const serverKey = agent.tools[0]?.server ?? `otc-order-${agent.key}`;
  const mcpServer = await prodPost("/api/integrations/mcp-servers", {
    name: `NovaTech ${agent.code} MCP Server`,
    description: `Mock MCP server for ${agent.name}`,
    url: `${CFG.prodUrl}/api/mock/${serverKey}`,
    status: "active",
    organizationId: CFG.prodOrgId,
  });
  console.log(`    ✓ MCP Server → ${mcpServer.id}`);
  ids.mcpServerId = mcpServer.id;

  // 6. Agent
  console.log("\n  6. Agent");
  const agentRecord = await prodPost("/api/agents", {
    name: agent.name,
    description: agent.description,
  ...
  console.log(`    ✓ Agent → ${agentRecord.id}`);
  ids.agentId = agentRecord.id;

  // 7. Agent → KB link
  console.log("\n  7. KB Link");
  const kbLink = await prodPost("/api/agents/" + ids.agentId + "/knowledge-bases", {
    knowledgeBaseId: ids.kbId,
  }).catch((e) => {
    console.warn(`    ⚠ KB link failed (may already exist): ${e.message}`);
    return { id: "skipped" };
  });
  console.log(`    ✓ KB Link → ${kbLink.id}`);

  // 8. Agent → MCP server link
  console.log("\n  8. MCP Server Link");
  const mcpLink = await prodPost("/api/agents/" + ids.agentId + "/mcp-servers", {
    mcpServerId: ids.mcpServerId,
  }).catch((e) => {
    console.warn(`    ⚠ MCP link failed (may already exist): ${e.message}`);
    return { id: "skipped" };
  });
  console.log(`    ✓ MCP Link → ${mcpLink.id}`);
