#!/usr/bin/env node
/**
 * LIT-AGT-010 — Continuation script
 * Runs Steps 9-11 (eval suite, outcome contract, final linking + ontology tags)
 * using IDs from the successful Steps 1-8 run.
 */
...
const ids = {
  ...
  kbSourceIds:     [],   // already created — not needed for remaining steps
  skillIds: [
...
const KPIS = [
  { name: "Leave Eligibility Determination Accuracy", unit: "percent", baseline: 78, target: 95, targetOperator: "gte", weight: 0.22, slaThreshold: 90, breachLevel: "critical", confidence: 0.90, trend: "improving", expression: "(correct_eligibility_determinations / total_determinations) * 100", measurement: "Validated against attorney-reviewed ground truth dataset covering all 50 states + 100+ municipalities, 3,500+ scenarios" },
  { name: "Concurrent Leave Calculation Accuracy", unit: "percent", baseline: 82, target: 97, targetOperator: "gte", weight: 0.18, slaThreshold: 92, breachLevel: "critical", confidence: 0.93, trend: "stable", expression: "(correct_concurrent_calculations / total_concurrent_calculations) * 100", measurement: "Compared against manual calculation by Littler leave attorneys on representative sample of 300 multi-law scenarios per quarter" },
  { name: "FMLA Notice Compliance Accuracy", unit: "percent", baseline: 88, target: 98, targetOperator: "gte", weight: 0.15, slaThreshold: 94, breachLevel: "critical", confidence: 0.92, trend: "stable", expression: "(compliant_notice_determinations / total_notice_determinations) * 100", measurement: "Validated against 29 C.F.R. §825.300 notice requirements checklist applied to generated notice guidance" },
...
async function main() {
  console.log("\n════════════════════════════════════════════════════════════════════");
  console.log("  LIT-AGT-010 — Continuation: Steps 9-11");
  console.log("════════════════════════════════════════════════════════════════════\n");
  ...
  console.log(`  Skills (6):      ${ids.skillIds.join(", ").slice(0, 60)}…`);
  console.log(`  Policies (6):    ${ids.policyIds.join(", ").slice(0, 60)}…`);
  console.log(`  Runbooks (6):    ${ids.runbookIds.join(", ").slice(0, 60)}…`);
...
main().catch(err => {
  console.error(`\n❌  Continuation failed: ${err.message}`);
  process.exit(1);
});
