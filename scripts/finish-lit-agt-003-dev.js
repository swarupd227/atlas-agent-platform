#!/usr/bin/env node
/**
 * LIT-AGT-003 — Finish Script
 * Steps 1-8 completed successfully. This script completes steps 9-11:
 *   - Eval suite (with schedule as string, not object)
 *   - Outcome contract + 8 KPIs
 *   - Link KB, outcome, eval suite to agent
 *   - Set ontology tags
 *   - Save all IDs to JSON
 *
 * Fix applied: schedule must be a string ("weekly:Wednesday:06:00 UTC"), not an object
 */
...
const ids = {
  ...
  kbSourceIds: [], // populated from creation output
};
...
const KPIS = [
  ...
  },
  {
    ...
    measurement: "Proportion of escalations validated by Littler investigation attorney as genuinely requiring attorney involvement; excludes mandatory escalations",
  },
];
...
async function main() {
  console.log("\n════════════════════════════════════════════════════════════════════");
  console.log("  LIT-AGT-003 — Finish Script (Steps 9-11)");
  console.log("  Completing eval suite, outcome contract, and linkages");
  console.log("════════════════════════════════════════════════════════════════════\n");

  // ── STEP 9: EVAL SUITE (schedule as string) ───────────────────────────────────
  step("9", "11", "Creating evaluation suite (schedule as string)…");
...
main().catch(err => {
  console.error(`\n❌  Finish script failed: ${err.message}`);
  process.exit(1);
});
