#!/usr/bin/env bash
# =============================================================================
# ATLAS — Order Validation & Promise Agent (OTC-AGT-002) — Production Migration
# Generated: 2026-03-30
#
# SINGLE COMMAND TO RUN:
#   bash migrate_otc_agt_002_to_prod.sh
#
# REQUIREMENTS: curl, jq
#
# This script recreates the full OTC-AGT-002 platform intelligence stack on
# the production environment — identical definitions to the dev provisioning
# with environment set to "production" and targeting the prod BASE_URL.
#
# Resources created:
#   6 Skills          (Customer Credit Check, Address Validation, Tax Calculation,
#                      ATP/Inventory Check, Fraud Detection, Order Enrichment)
#   1 Knowledge Base  (Order Validation & Promise KB)
#   6 Policies        (EAR/ITAR, OFAC, Sales Tax Nexus, PCI-DSS, SOX, KYC/AML)
#   1 Agent           (OTC-AGT-002, 20 tools, 14-node blueprint)
#   1 KB Link         (Agent → Knowledge Base)
#   1 Golden Dataset  (1000+ historical order validation benchmarks)
#   6 Runbooks        (Credit Down, Address Failure, Tax Timeout, Inventory Lag,
#                      Bulk Ingestion Failure, Order Stuck in Validation)
...
# =============================================================================
# STEP 4: Create Agent (base fields) — environment: production
# =============================================================================
...
jq -n \
  ...
  '{
    ...
    runtimeConfig: {
      prompt: "Validate an incoming order end-to-end through the Order Validation & Promise workflow for the Order-to-Cash process.\n\n## What You Do On Each Invocation\n\nYou receive an order payload from one of these sources: a quote acceptance handoff from OTC-AGT-001, an EDI 850 transaction from a trading partner, a customer portal order submission, or a manually entered order. The payload may be complete or partial — your first action is always enrichment.\n\n## Step-by-Step Execution\n\n**Step 1 — Enrich the Order**\nUse the Order Enrichment Skill to auto-populate any missing fields from customer master, product master, and contract terms. Confirm that all required fields are now populated before proceeding. If critical fields are still missing after enrichment, pause and request them from the order source.\n\n**Step 2 — Validate Customer Credit**\nUse the Customer Credit Check Skill to retrieve current credit limit and total exposure, check dunning level, and receive a credit decision. If HOLD: pause order, notify credit manager, route for credit override approval.\n\n**Step 3 — Check Inventory Availability**\nUse the ATP/Inventory Check Skill to query ATP for each order line, soft-reserve inventory with 30-minute TTL, identify backorder lines and present customer options.\n\n**Step 4 — Validate and Standardize Shipping Address**\nUse the Address Validation Skill to standardize the ship-to address, resolve tax jurisdiction code, flag residential addresses for carrier surcharge application.\n\n**Step 5 — Calculate Taxes**\nUse the Tax Calculation Skill to verify nexus status, check customer tax exemption certificate validity, calculate line-item taxes for all applicable jurisdictions.\n\n**Step 6 — Apply Payment Terms**\nApply contract-specific payment terms if contract is active; otherwise apply customer master default payment terms.\n\n**Step 7 — Screen for Fraud and Compliance Holds**\nUse the Fraud Detection Skill to run all checks in parallel: velocity check, address anomaly, first-order profile, quantity anomaly, denied party screening (BIS/DDTC/OFAC), OFAC country screening.\n\n**Step 8 — Commit Delivery Date**\nCalculate committed delivery date from ATP earliestShipDate + pick/pack lead time + carrier transit time.\n\n**Step 9 — Create Order in ERP/OMS**\nSubmit fully validated and enriched order to ERP order creation API. Attach all validation artifacts to order record.\n\n**Step 10 — Send Order Confirmation**\nSend structured confirmation to customer via their preferred channel.\n\n**Step 11 — Handoff to Fulfillment**\nEmit order.validated event to OTC-AGT-003 (Fulfillment & Exception Agent).",
...
cat > "$WORK/dataset.json" <<'ENDJSON'
{
  ...
  "coverageDimensions": [
    ...
    {"dimension": "Delivery Date Accuracy", "description": "Agent commits to a delivery date that is achievable within plus or minus 1 business day vs. manual reference", "targetPassRate": 0.96},
...
cat > "$WORK/rb1.json" <<'ENDJSON'
{
  ...
  "steps": [
    ...
    {"id": "s6", "label": "Manual override for high-value orders",  "type": "manual",       "description": "For orders > $50,000 during outage: route to Credit Manager for manual credit approval with offline data before order creation"},
  ...
  "approvalGates": [{"step": "s6", "role": "Credit Manager", "required": true}],
...
cat > "$WORK/rb2.json" <<'ENDJSON'
{
  ...
  "steps": [
    ...
    {"id": "s4", "label": "Notify customer for address correction",  "type": "notification","description": "If address is clearly wrong or undeliverable: contact customer requesting corrected ship-to address; provide 24-hour response window"},
  ...
  "approvalGates": [{"step": "s6", "role": "Order Operations Specialist", "required": true}],
...
cat > "$WORK/rb3.json" <<'ENDJSON'
{
  ...
  "approvalGates": [{"step": "s8", "role": "Tax Compliance Manager", "required": true}],
  "autonomyLevel": "semi_autonomous",
  "status": "active",
  "isPreBuilt": false,
  "severity": "medium",
...
cat > "$WORK/rb4.json" <<'ENDJSON'
{
  ...
  "steps": [
    {"id": "s1", "label": "Detect inventory data staleness",        "type": "detection",   "description": "Alert when ATP data age exceeds 15 minutes; compare WMS sync timestamp against current time; check WMS health endpoint"},
    {"id": "s2", "label": "Apply staleness threshold limits",       "type": "action",      "description": "Reduce available quantities by 10% safety buffer; flag all ATP decisions as STALE_DATA; set shorter soft-reservation TTL (15 min vs. 30 min)"},
    {"id": "s3", "label": "Alert fulfillment and logistics teams",  "type": "notification","description": "Notify fulfillment ops and logistics coordinators of ATP data staleness; advise to check physical inventory before picking any STALE_DATA orders"},
    ...
    {"id": "s8", "label": "Clear staleness flags and normalize",    "type": "resolution",  "description": "Remove STALE_DATA flags from orders processed during lag; restore standard reservation TTL; log incident metrics"}
  ],
  "approvalGates": [{"step": "s5", "role": "Fulfillment Coordinator", "required": true}],
  "autonomyLevel": "semi_autonomous",
  "status": "active",
  "isPreBuilt": false,
  "severity": "high",
...
cat > "$WORK/rb5.json" <<'ENDJSON'
{
  ...
  "steps": [
    {"id": "s1", "label": "Detect batch failure and scope",          "type": "detection",   "description": "Identify failure point in batch: pre-validation (parsing), validation failure (business rules), or ERP submission failure; determine count of failed vs. successful orders"},
    ...
    {"id": "s4", "label": "Generate partial processing report",      "type": "action",      "description": "Produce structured report: total submitted, successfully processed, failed with reasons, retry-eligible, requires-manual-review"},
    ...
    {"id": "s6", "label": "Route data-error failures to operations", "type": "escalation",  "description": "For orders failed due to data errors (missing fields, invalid SKUs, bad addresses): route to Order Operations queue with specific error detail"},
    {"id": "s7", "label": "Notify affected customers/EDI partners",  "type": "notification","description": "Send structured notification: orders received, orders processed, orders requiring correction with detail; provide resubmission instructions"},
    {"id": "s8", "label": "Process corrections and resubmissions",   "type": "action",      "description": "Accept corrected order resubmissions from customers/partners; process through normal validation workflow; confirm final order count"},
  ...
  "approvalGates": [{"step": "s4", "role": "Order Operations Manager", "required": true}],
  "autonomyLevel": "semi_autonomous",
  "status": "active",
  "isPreBuilt": false,
  "severity": "high",
...
cat > "$WORK/rb6.json" <<'ENDJSON'
{
  ...
  "description": "Timeout thresholds, auto-escalation, and manual release procedure for orders stuck in the validation workflow due to system hangs, human review queues exceeding SLA, or unresolvable validation loops.",
  "industry": "enterprise",
  "category": "escalation",
  "triggerType": "automated",
  ...
  "steps": [
    ...
    {"id": "s2", "label": "Classify stuck reason",                   "type": "detection",   "description": "Determine root cause: system hang (validation service unresponsive), human review queue backlog (reviewer not actioning), or infinite retry loop"},
    {"id": "s3", "label": "Auto-resume if system hang detected",     "type": "action",      "description": "If validation service hang: kill and restart the stuck validation process; replay from last successful checkpoint; do not re-run completed steps"},
    {"id": "s4", "label": "Escalate human review SLA breach",        "type": "notification","description": "If stuck due to human review queue: send reminder to assigned reviewer; if SLA breached > 2 hours, escalate to their manager with order context"},
    {"id": "s5", "label": "Notify customer of delay",                "type": "notification","description": "If validation stuck > 1 hour: send holding notification to customer acknowledging order received with estimated processing delay"},
    {"id": "s6", "label": "Manual release by Order Operations",      "type": "manual",      "description": "If auto-resume fails or root cause unresolved within 2 hours: Order Operations Specialist manually reviews validation state and resolves blocking issue"},
    {"id": "s7", "label": "Release soft inventory reservations",     "type": "action",      "description": "If order has been stuck for > reservation TTL (30 min): explicitly release soft inventory reservations to prevent permanent inventory lockup"},
    {"id": "s8", "label": "Complete validation or cancel order",     "type": "resolution",  "description": "After manual intervention: complete remaining validation steps and create order, or formally cancel with documented reason and customer notification"},
    {"id": "s9", "label": "Post-mortem and automation fix",          "type": "resolution",  "description": "Document root cause; if systematic issue: open engineering ticket for automation fix; update runbook with new detection thresholds if needed"}
  ],
  "approvalGates": [
...
# =============================================================================
echo "==================================================" >&2
echo " PRODUCTION MIGRATION COMPLETE" >&2
echo "==================================================" >&2
echo "" >&2
echo "Resource Summary (Production):" >&2
