  ...
  if (s === "C") {
    res.json({
      salesforce_case: {
        ...
        origin: "Astra Agents AI-First T1 Support",
...
router.post("/notify-am", (_req: Request, res: Response) => {
  ...
  if (s === "C") {
    res.json({
      am_notification: {
        ...
        message_preview: "Tyler — Astra Agents has opened SF Case #00078034 for BioNexus Pharma (Rachel Kim). ParityFactory sync daemon failure during FDA 21 CFR Part 11 validation window. Batch records LOT089-094 missing. FDA auditors arrive at 10:00 (4 hours). Legal hold active. Sofia Rodriguez assigned, P0 response in 30 min.",
  ...
  res.json({
    am_notification: {
      ...
      message_preview: "Hi James — Astra Agents has opened Salesforce Case #00074821 for Cascade Polymers (Priya Nair). InfinityQS v9.3 SPC chart outage affecting 47 control charts. ISO 9001 audit is tomorrow at 09:00. T2 specialist Marcus Chen assigned, response within 90 minutes.",
      notification_status: "delivered",
    },
    notified_at: new Date().toISOString(),
  });
