...
async function createOutcomeVersion(
  ...
  router.get("/api/outcomes/intelligence", async (req, res) => {
    try {
      ...
      res.json(assessOutcomeIntelligence(
        { agents: allAgents, templates: allTemplates, servers: allServers, tools: allTools, policies: allPolicies },
        {
          industry: industryStr,
          toolNames,
          roleNames,
          autonomyModes: autonomyList,
          riskTiers: riskList,
          proposedApprovalGatesCount: req.query.proposedApprovalGatesCount !== undefined
  ...
  router.post("/api/outcomes/from-proposal", checkPermission("create_modify_outcomes"), async (req, res) => {
    try {
      ...
      const body = req.body ?? {};
      const prepared = prepareOutcomeFromProposal(body, { baselineWhenMissing: 0 });
      const result = await createOutcomeFromProposal(orgId, actor, prepared, { source: body.source, evidence: body.evidence });
      res.status(201).json(result);
    } catch (e) {
      if (e instanceof OutcomeInputError) return res.status(400).json({ message: e.message });
      handleZodError(res, e);
    }
  });
  ...
  router.put("/api/kpis/:id/measurement", checkPermission("create_modify_outcomes"), async (req, res) => {
    ...
    try {
      const { kpi, describes } = await declareKpiMeasurement(actorFor(req), req.params.id as string, checked.source);
      res.json({ ...kpi, describes });
    } catch (e) {
   ...
   * from agent runs. Astra records one through the same function.
   */
  router.post("/api/kpis/:id/readings", checkPermission("create_modify_outcomes"), async (req, res) => {
    ...
    try {
      const recorded = await recordKpiReading(actorFor(req), req.params.id as string, {
