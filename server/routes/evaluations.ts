...
export default function createEvaluationsRouter(industryEvalFrameworks: Record<string, any>) {
  ...
  router.post("/api/blueprints/:id/compile", checkPermission("create_modify_blueprints"), async (req, res) => {
    ...
    if (linkedAgentForCompile?.agentType === "team") {
      ...
      const { errors: teamErrors, warnings: teamWarnings } = validateTeamGraph(linkedAgentForCompile, teamNodes, teamEdges);
...
- If the user describes a use case, suggest a complete agent configuration they can use.`;
  ...
  // AI Agent Drafter — "describe an outcome, get a governed agent draft to review".
  // Unlike /api/ai/propose-agents (outcome/team-bound, multi-agent), this drafts a
  // SINGLE free-standing agent from a plain-language description with no
  // outcomeContract/KPIs required. Draft is scoped to real, ranked platform
  // resources (MCP tools, skills, active policies, templates, ontology concepts)
  // so the LLM can't invent tool/policy names — and is re-checked deterministically
  // against active org policies before returning (never trust the LLM's own
  // compliance claim, same discipline as propose-agents' conflict pass). This
  // endpoint never writes to the DB; the caller reviews/edits the draft in the
  // wizard and creates the agent through the normal POST /api/agents flow.
  router.post("/api/ai/draft-agent", checkPermission("create_modify_blueprints"), async (req, res) => {
    try {
      if (!process.env.AI_INTEGRATIONS_ANTHROPIC_API_KEY && !process.env.ANTHROPIC_API_KEY) {
        return res.status(503).json({ error: "AI assistant is not configured" });
      }
