...
Complete all 3 steps. Compute scorecard-indicated rating and gap vs. current rating. All output is model-indicated only — not a rating opinion.`,
  ...
  router.get("/demo-api/hearst/revenue", async (_req, res) => {
    ...
    const aiInsights = [
      ...
      { type: "warning", icon: "⚠️", title: "Esquire readership segment drift detected", body: "Engagement data shows Esquire's active readers are skewing toward Career & Finance over traditional Style content. Astra Agents is automatically redistributing Esquire subscriber allocation before unsubscribe rates signal the shift.", metric: "Segment Drift", urgency: "high" },
  ...
  // Eager agent provisioning at route registration — agents appear in registry immediately.
  ensureOnespanAgents().catch(err => console.error("[onespan] Eager init failed:", err));
  ...
  async function buildStagePatch(stage: string): Promise<Record<string, unknown>> {
    ...
    if (stage === "diagnosed") {
      patch.diagnosisDetails = {
        ...
        detectionLatency: "4 minutes (vs ~2.5 hours without Astra Agents monitoring)",
      };
  ...
  async function buildFinStagePatch(stage: string): Promise<Record<string, unknown>> {
    ...
    if (stage === "diagnosed") {
      ...
      patch.businessImpact = {
        ...
        falseNegativesPerHour: 2,
        financialExposure: "$284K capped vs est. $1.4M without Astra Agents",
