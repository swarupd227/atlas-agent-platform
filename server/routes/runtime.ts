...
// In-memory store for completed export jobs.
// Files are stored here instead of being embedded in the SSE done event,
// which avoids sending hundreds of KB through the SSE stream (causes blank screen in production).
// Entries expire after 10 minutes — the client fetches immediately after the stream ends.
const exportJobResults = new Map<string, { files: Record<string, string>; expiresAt: number }>();
setInterval(() => {
  const now = Date.now();
  for (const [key, val] of exportJobResults) {
    if (val.expiresAt < now) exportJobResults.delete(key);
  }
}, 60_000).unref();

// GET /api/export-result/:jobId — retrieve files generated by an export job
router.get("/api/export-result/:jobId", (req, res) => {
  const result = exportJobResults.get(req.params.jobId);
  if (!result) return res.status(404).json({ error: "Export result not found or expired" });
  exportJobResults.delete(req.params.jobId);
...
`;
      ...
      // Store files server-side; send only a jobId in the SSE done event.
      // Sending all file contents through SSE (potentially 200-500KB) causes the stream
      // to fail silently in production nginx, resulting in a blank preview.
      const jobId = crypto.randomUUID();
      exportJobResults.set(jobId, { files, expiresAt: Date.now() + 10 * 60 * 1000 });
      emit("done", {
        jobId,
        metadata: {
          agentName: agent.name,
  ...
  router.post("/api/agents/:id/export-code/bundle", async (req, res) => {
    ...
    try {
      ...
      // Store files server-side; emit only jobId in the done event (not the full file map).
      const bundleJobId = crypto.randomUUID();
      exportJobResults.set(bundleJobId, { files, expiresAt: Date.now() + 10 * 60 * 1000 });
      emit("done", {
        jobId: bundleJobId,
        metadata: {
          bundled: true,
