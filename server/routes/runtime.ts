...
// (multiple server instances) all read from the same store — the original Map caused
// 404s when the SSE response was written by instance A but the client polled instance B.
// Entries expire after 10 minutes; the client fetches immediately after the stream ends.

async function storeExportJob(jobId: string, files: Record<string, string>): Promise<void> {
  const expiresAt = new Date(Date.now() + 10 * 60 * 1000).toISOString();
  await db.execute(sql`
    INSERT INTO export_jobs (id, files_json, expires_at)
    VALUES (${jobId}, ${JSON.stringify(files)}, ${expiresAt}::timestamptz)
    ON CONFLICT (id) DO UPDATE
      SET files_json = EXCLUDED.files_json,
          expires_at = EXCLUDED.expires_at
  `);
  // Opportunistically prune old rows (fire-and-forget — never blocks the export response)
  db.execute(sql`DELETE FROM export_jobs WHERE expires_at < NOW()`).catch(() => {});
}

async function getAndDeleteExportJob(jobId: string): Promise<Record<string, string> | null> {
  const result = await db.execute(sql`
    DELETE FROM export_jobs
    WHERE id = ${jobId} AND expires_at > NOW()
    RETURNING files_json
  `);
  if (!result.rows || result.rows.length === 0) return null;
  try {
    return JSON.parse((result.rows[0] as { files_json: string }).files_json);
  } catch {
    return null;
  }
}

// GET /api/export-result/:jobId — retrieve files generated by an export job
router.get("/api/export-result/:jobId", async (req, res) => {
  try {
    const files = await getAndDeleteExportJob(req.params.jobId);
    if (!files) return res.status(404).json({ error: "Export result not found or expired" });
    res.json({ files });
  } catch (e) {
    console.error("[export-result] DB lookup failed:", e);
    res.status(500).json({ error: "Failed to retrieve export result" });
  }
});
...
`;
        ...
        // NOTE: aiResult?.entrypoint is intentionally NOT used here.
        // The agent's runtime format may be TypeScript, but agent.py must always be
        // a Python file following the MLflow 2.22.4 two-block structure. Using the
        // AI-generated entrypoint would produce TypeScript code in a .py file, causing
        // all 9 Python-specific guide checks to fail (issues #7, #8, #11–#13, #16–#18).
        // The static template below already has all fixes correctly implemented.
        files["agent.py"] = (
...
`;
        ...
        files[".env.example"] = [
          ...
          "MLFLOW_TRACKING_URI=databricks",
          "",
          "# Unity Catalog namespace for model registration — replace <catalog>.<schema> in agent.py",
          "UC_CATALOG=",
          "UC_SCHEMA=",
        ].join("\n") + "\n";
...
`;
      ...
      // Store files in PostgreSQL (shared across all autoscale instances).
      // Previously used an in-memory Map which caused 404s when the SSE stream completed
      // on instance A but the client polled GET /api/export-result/:jobId on instance B.
      const jobId = crypto.randomUUID();
      await storeExportJob(jobId, files);
      emit("done", {
        jobId,
        metadata: {
          agentName: agent.name,
    ...
    try {
      ...
      async function buildAgentFiles(
        ...
        if (framework === "claude-code") {
          ...
          // Databricks Mosaic AI — ResponsesAgent (Python only, Databricks Serverless)
          const agentSlugDbx = (agentRec.name || "agent").toLowerCase().replace(/[^a-z0-9]+/g, "_");
          const dbxClassName = agentSlugDbx.replace(/(?:^|_)([a-z])/g, (_: string, c: string) => c.toUpperCase());
          const dbxEndpoint = resolvedBundleProvider === "openai" ? "databricks-meta-llama-3-3-70b-instruct" : "databricks-claude-3-5-sonnet";
          const pin = pinVersions;
          const systemPromptShort = (systemPrompt || "").substring(0, 500).replace(/`/g, "'").replace(/\\/g, "\\\\");
          ...
          // NOTE: aiResult?.entrypoint intentionally NOT used — same reason as single-agent export.
          // agent.py must always be Python (MLflow 2.22.4 two-block structure), regardless of
          // whether the agent's runtime format is TypeScript. See comment at line ~4922.
          localFiles[`${dirPrefix}/agent.py`] = (
            `# ─── Standard library ONLY at module top level ─────────────────────────────\n` +
            `import json\n` +
            `import os\n` +
            `import sys\n` +
          ...
          const dbxToolInfoEntries = tools.map(t => buildDbxToolInfoEntry(t)).join("\n");
          localFiles[`${dirPrefix}/tools/__init__.py`] = (
            `# AgentBricks Tool Registry — ToolInfo pattern for ResponsesAgent\n` +
            `# Generated for: ${agentRec.name}\n` +
            `${dbxImportLines}\n\n\n` +
            `ToolInfo = namedtuple("ToolInfo", ["name", "spec", "execute"])\n\n` +
          ...
          localFiles[`${dirPrefix}/pyproject.toml`] = (
            `[project]\n` +
            `name = "${agentSlugDbx}"\n` +
            `version = "0.1.0"\n` +
      ...
      // Databricks Asset Bundle — bundle-level databricks.yml for Databricks framework
      if (framework === "databricks") {
        emit("progress", { phase: "bundle", message: "Generating databricks.yml (Databricks Asset Bundle)..." });
        const orchSlugDbx = orchestrator.name.toLowerCase().replace(/[^a-z0-9]+/g, "_");
        const agentJobEntries = allSlots.map(slot => {
          const slug = slot.agent.name.toLowerCase().replace(/[^a-z0-9]+/g, "_");
          return (
            `    ${slug}_deploy:\n` +
            `      name: "Deploy ${slot.agent.name}"\n` +
            `      tasks:\n` +
            `        - task_key: log_model\n` +
            `          spark_python_task:\n` +
            `            python_file: \${workspace.file_path}/${slot.prefix}/agent.py\n` +
            `          environment_key: default\n` +
            `      environments:\n` +
            `        - environment_key: default\n` +
            `          spec:\n` +
      ...
      const serviceEntries = allSlots.map(slot => {
        ...
        const startCmd = framework === "claude-code"
          ? `["npm", "start"]`
          : framework === "databricks" ? `["python", "agent.py"]`
          : format === "typescript" ? `["npm", "start"]` : `["python", "src/runtime/orchestrator.py"]`;
        const llmKey = framework === "databricks" ? "DATABRICKS_TOKEN" : resolvedBundleProvider === "openai" ? "OPENAI_API_KEY" : "ANTHROPIC_API_KEY";
      ...
      await storeExportJob(bundleJobId, files);
      emit("done", {
        jobId: bundleJobId,
        metadata: {
          bundled: true,
...
Include 5-8 steps with at least one approval gate. Make steps industry-specific with proper regulatory language.`;
  ...
  router.post("/api/pipeline-runs/:id/advance", async (req, res) => {
    ...
    if (nextIdx < stages.length) {
      ...
      if (nextStage.stageType === "approval_gate") {
        ...
        const interruptDef = await storage.getInterruptDefinitionByStage(run.pipelineId, nextStage.id);
  ...
  router.post("/api/pipeline-runs/:id/resume", async (req, res) => {
    try {
      ...
      if (run.status !== "paused_at_gate") {
        return res.status(400).json({ error: "Run is not paused at an approval gate" });
      }
      const pipeline = await storage.getAgentPipeline(run.pipelineId);
      if (!pipeline) return res.status(404).json({ error: "Pipeline not found" });
      ...
      console.error("[pipeline-runs/resume] Error:", e);
      res.status(500).json({ error: e instanceof Error ? e.message : "Resume failed" });
    }
  });

  // ── Interrupt Instances: list for a run ───────────────────────────────────
  router.get("/api/pipeline-runs/:id/interrupt-instances", async (req, res) => {
    try {
  ...
  router.post("/api/interrupt-definitions", async (req, res) => {
    try {
      ...
      const def = await storage.createInterruptDefinition(data);
      res.status(201).json(def);
    } catch (e) {
      if (e instanceof ZodError) return res.status(400).json({ error: "Validation error", errors: (e as ZodError).errors });
      res.status(500).json({ error: "Failed to create interrupt definition" });
    }
  });

  router.get("/api/interrupt-definitions/:id", async (req, res) => {
    try {
  ...
  router.put("/api/interrupt-definitions/:id", async (req, res) => {
    try {
      const defSchema = z.object({
        name: z.string().min(1).optional(),
        title: z.string().optional(),
  ...
  router.delete("/api/interrupt-definitions/:id", async (req, res) => {
    try {
      const ok = await storage.deleteInterruptDefinition(req.params.id);
      if (!ok) return res.status(404).json({ error: "Interrupt definition not found" });
      res.json({ success: true });
    } catch (e) {
      res.status(500).json({ error: "Failed to delete interrupt definition" });
    }
  });
    ...
    try {
      ...
      if (currentStage.stageType === "composite") {
        ...
        const initialState: Record<string, any> = {
          ...
          ...(stateEnabled ? currentStateObj : previousResults),
        };
