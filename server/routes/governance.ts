...
Ontology: ${ontologyName || "industry standard"}`,
  ...
  router.patch("/api/approvals/:id", async (req, res, next) => {
    ...
    // Everything a decision does beyond the approval row -- outcome review, team-run and
    // Workspace-run resume, code execution, screenshot baseline, patch rollout, deployment
    // activation, retirement -- lives in server/approval-decision.ts, shared with the
    // Astra Workspace so a decision made there has exactly the same effect.
    await applyApprovalEffects(approval, status, {
      orgId: getOrgId(req) ?? getDefaultOrgId() ?? undefined,
      requestOrgId: getOrgId(req),
      decidedBy,
      constraintsJson,
    });
