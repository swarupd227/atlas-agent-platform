...
export interface DraftedAgentResult {
  draft: any;
  policyConflicts: Array<{ severity: "warn" | "error"; message: string; policyId?: string; policyName?: string }>;
  candidateContext: {
    mcpServersConsidered: number;
    skillsConsidered: number;
    policiesConsidered: number;
    templatesConsidered: number;
    ontologyConceptsConsidered: number;
  };
}

// Core drafting logic behind POST /api/ai/draft-agent, extracted so the
// process-flow sync endpoint (server/routes/outcomes.ts) can draft a single
// agent for a changed/added process-flow node without going back through
...
Return ONLY a JSON object with exactly these fields: name, description, riskTier, autonomyMode, systemPrompt, toolsConfig, mcpServerNames, policyBindings, ontologyTags, preloadedSkills, guardrailsConfig, evalSuiteConfig, sourceTemplateId, reasoning.`;
  ...
  try {
    ...
    throw new Error("Agent draft generation failed: the model's response wasn't valid JSON.");
  }
  ...
  if (!validation.success) {
    console.error("[draftSingleAgent] LLM response failed schema validation:", validation.error.message);
    throw new Error(`Agent draft generation failed: invalid response structure (${validation.error.message})`);
  }
