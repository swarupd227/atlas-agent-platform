  ...
  return [
    {
      ...
      description: `Execute a read-only SQL SELECT query against the connected ${dialectLabel} database and return structured rows with column types. DDL/DML statements are blocked. Results are truncated at 1,000 rows — use LIMIT and WHERE filters. This connection may be scoped to a fixed set of tables (governance); a query outside that set is rejected, not silently filtered. Prefer running sql_describe_table on any table you haven't already inspected this session before querying it — text columns often have data-quality issues (inconsistent casing, stray whitespace) that a blind query will get wrong. When the user's question names a specific status/category value (e.g. "open" tickets), filter on that exact distinct value from column_data_quality/sql_describe_table first — do not silently fold in related-but-different values (e.g. "In Progress") to make the answer more complete; if you do broaden the filter, say so plainly in your first sentence, not buried in a caveat. When a table exposes both an id column and a joinable human-readable name/label for the same entity (e.g. an agent_id with a matching name in another table), join to and report the name — bare ids make an answer far less useful unless the user explicitly asked for ids.`,
      inputSchema: {
        type: "object",
        properties: {
