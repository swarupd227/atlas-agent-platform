...
export const INTEGRATION_REGISTRY: IntegrationDef[] = [
  ...
  },
  {
    ...
    oauthConfig: {
      ...
      requiredPermissions: [
        ...
        { permission: "Files.ReadWrite.All",   type: "Delegated",   description: "Read SharePoint and OneDrive files, and change their sharing (needed only by governance remediation tools)" },
    ...
    wave: 3,
    capabilities: ["read_email", "send_email", "read_calendar", "create_events", "read_users", "send_teams_messages", "read_sharepoint", "read_onedrive", "reconcile_duplicates", "create_planner_tasks"],
  ...
  // ── Wave 4: Data & ERP ────────────────────────────────────────────────────
  {
    ...
    credentialFields: [
      ...
      { key: "private_key", label: "Private Key (PEM) — key-pair auth", type: "password", required: false, placeholder: "-----BEGIN PRIVATE KEY-----" },
      { key: "password", label: "Password — username/password auth (if not using key-pair)", type: "password", required: false },
  ...
  },
  {
    ...
    authMethod: "basic",
    credentialFields: [
      { key: "system_type", label: "System Type (s4hana or b1)", type: "text", required: true, placeholder: "s4hana" },
      { key: "base_url", label: "OData / Service Layer Base URL", type: "url", required: true, placeholder: "https://myhost:443/sap/opu/odata" },
      ...
      { key: "access_token", label: "OAuth2 Bearer Token — S/4HANA OAuth2 mode", type: "password", required: false },
      { key: "client", label: "SAP Client Number", type: "text", required: false, placeholder: "100" },
    ],
